...
func (b *ByocDo) processServiceLogs(ctx context.Context) (string, error) {
	...
	for _, app := range currentApps {
		...
		if app.Spec.Name == mainAppName {
			...
			deployments, _, err := b.driver.Client.Apps.ListDeployments(ctx, app.ID, &godo.ListOptions{})
			if err != nil {
				return "", err
			}

			mainDeployLogs, resp, err := b.driver.Client.Apps.GetLogs(ctx, app.ID, "", "", godo.AppLogTypeDeploy, true, 50)
			if resp.StatusCode != 200 {
				...
				if deployments[0].Phase == godo.DeploymentPhase_Error && deployments[0].UpdatedAt.After(time.Now().Add(-2*time.Minute)) {
					failDeployLogs, _, err := b.driver.Client.Apps.GetLogs(ctx, app.ID, deployments[0].ID, "", godo.AppLogTypeDeploy, true, 50)
					if err != nil {
						return "", err
					}
			...
			mainRunLogs, resp, err := b.driver.Client.Apps.GetLogs(ctx, app.ID, "", "", godo.AppLogTypeRun, true, 50)
			if resp.StatusCode != 200 {
				// Assume no deploy happened, return without an error
				return "", nil
			}
