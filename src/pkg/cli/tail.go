...
func Tail(ctx context.Context, provider client.Provider, projectName string, options TailOptions, handler LogEntryHandler) error {
	...
	return streamLogs(ctx, provider, projectName, options, handler)
}
...
func TailEntryHandler(e *defangv1.LogEntry, options *TailOptions) error {
	if options.Raw {
		if e.Stderr {
