...
func (m *deploymentModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case serviceUpdate:
		m.services[msg.name].status = msg.status
		return m, nil
...
func (m *deploymentModel) View() string {
	...
	for _, name := range serviceNames {
		...
		if svc.status == defangv1.ServiceState_DEPLOYMENT_COMPLETED {
			spinnerOrCheck = "✓ "
		} else if svc.status == defangv1.ServiceState_DEPLOYMENT_FAILED {
			spinnerOrCheck = "✗ "
		}
...
func MonitorWithTUI(ctx context.Context, project *compose.Project, provider client.Provider, waitTimeout time.Duration, deploymentID string) (map[string]defangv1.ServiceState, error) {
	...
	go func() {
		...
		if monitorErr == nil {
			// prevent leaving partial state and spinners on screen after successful completion
			for _, serviceName := range servicesNames {
				if serviceStates[serviceName] == defangv1.ServiceState_DEPLOYMENT_PENDING {
					p.Send(serviceUpdate{
						name:   serviceName,
						status: defangv1.ServiceState_DEPLOYMENT_COMPLETED,
					})
				}
			}
		}
