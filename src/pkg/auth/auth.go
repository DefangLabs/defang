...
func Poll(ctx context.Context, key string) ([]byte, error) {
	...
	for {
		...
		if err != nil {
			...
			// retriablehttp would return a "giving up after X attempt(s)" error, but no retry if the context was done
			var urlError *url.Error
			if errors.As(err, &urlError) && strings.Contains(err.Error(), "giving up after") && ctx.Err() == nil {
