...
func NewDebugger(ctx context.Context, addr string, providerID *client.ProviderID) (*Debugger, error) {
	agent, err := agent.New(ctx, addr, providerID, agent.DefaultSystemPrompt)
	if err != nil {
		return nil, err
	}
	return &Debugger{
