...
func generateComposeFile(claude *ClaudeClient, systemInstructions, prompt string) (string, error) {
	...
	codeFenceStart := "```yaml"
	codeFenceEnd := "```"
	startIndex := bytes.Index([]byte(responseText), []byte(codeFenceStart))
	if startIndex == -1 {
		return "", fmt.Errorf("no code fence found in Claude response")
	}
	startIndex += len(codeFenceStart)
	endIndex := bytes.Index([]byte(responseText[startIndex:]), []byte(codeFenceEnd))
	if endIndex == -1 {
		return "", fmt.Errorf("no closing code fence found in Claude response")
	}
	endIndex += startIndex
