...
func TestHandleMessage(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "HandleMessage with tool calls",
			maxTurns: 2,
			generatorResponses: []*ai.ModelResponse{
				{
			...
			expectedResponseMessages: []*ai.Message{
				...
				ai.NewMessage(ai.RoleTool, nil,
					ai.NewToolResponsePart(&ai.ToolResponse{
						Name:   "read_file",
						Ref:    "",
						Output: "error calling tool read_file: open value1: no such file or directory",
					}),
				),
				ai.NewModelMessage(
		...
		{
			name:     "HandleMessage with tool calls in both responses",
			maxTurns: 2,
			generatorResponses: []*ai.ModelResponse{
				{
					Message: ai.NewModelMessage(
			...
			expectedResponseMessages: []*ai.Message{
				...
				ai.NewMessage(ai.RoleTool, nil,
					ai.NewToolResponsePart(&ai.ToolResponse{
						Name:   "read_file",
						Ref:    "",
						Output: "error calling tool read_file: open value2: no such file or directory",
					}),
				),
			},
