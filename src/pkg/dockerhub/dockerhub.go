...
func GenerateNewPAT(ctx context.Context, label string) (string, string, error) {
	...
	var pat string
	if !strings.HasPrefix(auth.Password, "dckr_pat_") {
		// Create a new DockerHub PAT
		var docHubClient DockerHubClient
		if err := docHubClient.Login(ctx, auth.Username, auth.Password); err != nil {
	...
	if err := ValidatePATWithRepo(auth.Username, pat, "library/alpine"); err != nil {
		return "", "", ErrInvalidCredential
	}
	return auth.Username, pat, nil
}
...
func ValidatePATWithRepo(user, pat string, repo string) error {
	...
	fmt.Printf("Requesting token for repo %s from URL: '%s'\n", repo, url)
