...
type StacksManager interface {
	TargetDirectory() string // test only
...
func (sl *SessionLoader) loadStack(ctx context.Context) (*stacks.Parameters, string, error) {
	if sl.sm == nil {
		// Without a stack manager, we can only load from the default options (env vars or flags)
		return stacks.GetFallbackStack(sl.opts.Default)
	}
