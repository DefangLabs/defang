...
func New(stack string, region aws.Region) *AwsEcs {
	return &AwsEcs{
		stack:     stack,
		region:    region,
...
func (a *AwsEcs) SetUp(ctx context.Context, image string, color Color) error {
	...
	res, err := s.Up(ctx, optupColor(color), optup.ProgressStreams(os.Stdout))
	if err != nil {
		return err
	}
